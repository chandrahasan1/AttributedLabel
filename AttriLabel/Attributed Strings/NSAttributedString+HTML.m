...
- (id)initWithHTML:(NSData *)data options:(NSDictionary *)options documentAttributes:(NSDictionary **)dict
{
    ...
    /*
    NSString *rawString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease];
    if ([rawString isEqualToString:@"esign to-dos<br><br><ol><li>I up</li></ol>"])
        NSLog(@"%@", rawString);
    	
    if ([rawString isEqualToString:@"<p>Ashish,</p><p><br></p><p>The front</p>"])
        NSLog(@"%@", rawString);
    
    if ([rawString isEqualToString:@"Ashish<br><br>The frontaa"])
        NSLog(@"%@", rawString);

    if ([rawString isEqualToString:@"are currently not logged).<br><br><b>Server migration</b><br>So to move the serv"])
        NSLog(@"%@", rawString);
    
    if ([rawString isEqualToString:@"To stop the server:<br><pre>sudo su</pre>"])
        NSLog(@"%@", rawString);
    
    if ([rawString isEqualToString:@"<pre>attach<br></pre>And"])
        NSLog(@"%@", rawString);

    if ([rawString rangeOfString:@"Check out the pull and release"].location != NSNotFound)
        NSLog(@"%@", rawString);

    if ([rawString rangeOfString:@"This is a quote that goes in the middle of a regular paragraph."].location != NSNotFound)
        NSLog(@"%@", rawString);
    */
    
	// Make it a string
    NSString *raw = [[NSString alloc] initWithPotentiallyMalformedUTF8Data:data];
	NSString *htmlString = [[raw stringByReplacingOccurrencesOfString:@"&#160;" withString:@""] retain];
	[raw release];
    ...
    CGFloat defaultFontSize = [[options objectForKey:DTDefaultFontSize] floatValue];
		...
		if ([scanner scanHTMLTag:&tagName attributes:&tagAttributesDict isOpen:&tagOpen isClosed:&immediatelyClosed] && tagName)
		{
			...
			else if ([tagName isEqualToString:@"li"]) 
			{
				if (tagOpen)
				{
...
#else
					CGFloat indentSize = 20.0 * textScale;
#endif
                    ...
                    currentTag.paragraphStyle.paragraphSpacingBefore = 6.0f;
			...
			else if ([tagName isEqualToString:@"ol"]) 
			{
				if (tagOpen)
				{
                    DTHTMLElement *parent = currentTag.parent;
                    // Sometimes the ol and ul tags are not wrapped inside a div tag,
                    // This takes care of that
                    if (parent.tagName == nil)
                    {
                        if (![previousTag isEqualToString:@"br"])
                        {
			...
			else if ([tagName isEqualToString:@"pre"])
			{
				if (tagOpen)
				{
                    ...
                    CGFloat indentSize = 10.0 * textScale;
                ...
                {
                    /*
                     <div>Rishabh, Vijay - Probably an inline message saying<br><br><pre>Sorry, hike doesn't support international SMS just yet. This won't be for long though! Happy hiking.</pre>
                     <br>Ashish will provide UI. Let's not ask him to invite to hike again, since its all over the place already.</div>
                     */
                    /*
                     Testing a brand new message. This is just normal text.<br><br><pre>This is a quote that goes in the middle of a regular paragraph.</pre><br>And this is a follow up para. 
                     */
			...
			{
				if (tagOpen && ![previousTag isEqualToString:tagName])
				{
					currentTag.paragraphStyle.paragraphSpacing = defaultFontDescriptor.pointSize;
				}
				
			}
            else if ([tagName isEqualToString:@"div"] && !tagOpen)
            {
                if (currentTag.text.length >0)
                {
                    [tmpString appendNakedString:UNICODE_LINE_FEED];  
                }
                else if (![previousTag isEqualToString:@"ul"] || [previousTag isEqualToString:@"ol"]) // Because lists already do that
                {
                    [tmpString appendNakedString:@"\n"];
                }
            }
			else if ([tagName isEqualToString:@"br"])
			{
                ...
                if ([previousTag isEqualToString:@"br"] || [previousTag isEqualToString:@"p"]
                    || [previousTag isEqualToString:@"ul"] || [previousTag isEqualToString:@"ol"])
                {
                    currentTag.text = @"";
                    [tmpString appendNakedString:@"\n"];
                }
                else if (previousTag == nil || [previousTag isEqualToString:@"b"]  || [previousTag isEqualToString:@"pre"])
                {
			...
			else if (!tagOpen)
			{
				...
				if (![currentTag isInline] && ![currentTag isMeta] && ![[tmpString string] hasSuffix:@"\n"] && ![[tmpString string] hasSuffix:UNICODE_OBJECT_PLACEHOLDER] && ![tagName isEqualToString:@"div"] && ![tagName isEqualToString:@"p"]  && ![tagName isEqualToString:@"pre"])
				{
		...
		else 
		{
			...
			if ((skippedAngleBracket||[scanner scanUpToString:@"<" intoString:&tagContents]) && !currentTag.tagContentInvisible)
			{
				...
				if ([[tagContents stringByTrimmingCharactersInSet:[NSCharacterSet newlineCharacterSet]] length])
				{
                    ...
                    // We do this else when detecting links the plainText will be ...sometexthttp://apple.comremaining...
                    // And so the links will not get detected it unless there is space like ...sometext http://apple.com remaining...
					if (currentTag.link)
                        tagContents = [NSString stringWithFormat:@" %@ ", tagContents];
    ...
    if ([rawString isEqualToString:@"esign to-dos<br><br><ol><li>I up</li></ol>"])
    {
        NSString *finalStr = [tmpString string];
        NSLog(@"%@", finalStr);
    }
    ...
    if ([rawString rangeOfString:@"Booaaah"].location != NSNotFound)
        NSLog(@"%@", rawString);
